...
{
	if (new_pos.row < 0 || new_pos.row >= game->map->rows || new_pos.col < 0
		|| new_pos.col >= game->map->cols)
		return (0);
	if (game->map->grid[new_pos.row][new_pos.col] == '1')
		return (0);
	if (game->map->grid[new_pos.row][new_pos.col] == 'E'
		...
		t_position new_pos)
{
	...
	mlx_put_image_to_window(game->mlx, game->win, game->grass, old_pos.col * 64,
		old_pos.row * 64);
	mlx_put_image_to_window(game->mlx, game->win, game->player, new_pos.col
		* 64, new_pos.row * 64);
